...
class Mpc_nn:
    ...
    def MPCobj_nn(self, u_hat, u_window, y_window, sp):
        ...
        if self.multistep == 0:
            ...
            Obj = np.sum(W_CV.dot((pred_nn["y_hat"] - SP_hat)**2)) + np.sum(W_MV.dot(((u_hat0[1:] - u_hat0[0:-1])**2)))

        else:
            pred_nn["y_hat_multi"] = Ytu
            # pred_nn["u_hat_multi"] = Xtu[self.window:,0]
            
            Obj = np.sum(((pred_nn["y_hat_multi"] - SP_hat)**2).dot(W_CV)) + np.sum(((u_hat0[1:] - u_hat0[0:-1])**2).dot(W_MV))
        print('Obj=',Obj)
        return Obj
    ...
    # MPC calculation
        ...
        solution = minimize(self.MPCobj_nn, uhat, method='SLSQP',bounds=bnds,args=(u_window, y_window, sp),options={'eps': 1e-06,
                                                                                                                    'maxiter': 100,
                                                                                                                    'ftol': 1e-03})
        # 
