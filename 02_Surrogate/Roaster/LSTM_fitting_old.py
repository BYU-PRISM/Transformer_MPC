...
file = 'cstr_nonlinearity.csv'
train = pd.read_csv(file)

train.Ca[1:np.shape(train)[0]] = train.Ca[0:np.shape(train)[0]-1]


# Scale features
s1 = MinMaxScaler(feature_range=(-1,1))
Xs = s1.fit_transform(train[['Ca','Tc']])

# Scale predicted value
s2 = MinMaxScaler(feature_range=(-1,1))
Ys = s2.fit_transform(train[['Ca']])
...
plt.plot(train['Time'][window:],Ym,'ko',label='Measured')
plt.ylabel('Temperature (°C)')
plt.legend()
plt.subplot(2,1,2)
plt.plot(train['Tc'],label='heater (%)')
plt.legend()
...
test = pd.read_csv('cstr_nonlinearity_validate.csv')
# test = test[0::5]

test.Ca[1:np.shape(test)[0]] = test.Ca[0:np.shape(test)[0]-1]

Xt = test[['Ca','Tc']].values
Yt = test[['Ca']].values
...
plt.ylabel('Temperature (°C)')
plt.subplot(2,1,2)
plt.plot(test['Time'][window:],test['Tc'][window:],'b.',label='Heater')
plt.xlabel('Time (sec)'); plt.ylabel('Heater (%)')
plt.legend()
plt.grid()
