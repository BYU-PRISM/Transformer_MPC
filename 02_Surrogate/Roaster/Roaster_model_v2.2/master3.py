...
#%% Running LSTM model
    if i <= window:
        pred.Ore_amps = np.ones(P)*data_input["Ore_amps"][i]
        pred.Sulfur_tph = np.ones(P)*data_input["Sulfur_tph"][i]
        pred.O2_scfm = np.ones(P)*data_input["O2_scfm"][i]
        pred.Carbon_in = np.ones(P)*data_input["Carbon_in"][i]
        pred.Sulf_in = np.ones(P)*data_input["Sulf_in"][i]
        pred.CO3_in = np.ones(P)*data_input["CO3_in"][i]
        pred.O2 = np.ones(P)*O2[i]
        pred.CO2 = np.ones(P)*CO2[i]
        pred.SO2 = np.ones(P)*SO2[i]
        pred.TCM = np.ones(P)*TCM[i]
        pred.FeS2 = np.ones(P)*FeS2[i]
        pred.CaCO3 = np.ones(P)*CaCO3[i]
        pred.T_1 = np.ones(P)*T_1[i]
        pred.T_2 = np.ones(P)*T_2[i]
        ...
        # ctl = 0
        pred = lstm_prediction.rto(past, Dof, tail, window, sp[i,:], v, s1, P, M, ctl, umeas, nCV)
        
        if ctl == 1:
            data_input["Ore_amps"][i+1] = pred.Ore_amps[0]
            data_input["Sulfur_tph"][i+1] = pred.Sulfur_tph[0]
            data_input["O2_scfm"][i+1] = pred.O2_scfm[0]
